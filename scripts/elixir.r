...
# remove!
args <- c("../data/elixir_2021-09-22_11-58_dig_analysis.tsv","elixir", "../elixir_community.txt")
# END remove!
user_prefix <- args[2]
...
categories_colors <- tibble(category=c("Compute","Tools","FAIR definition","Data","Research"),
                            color=c("#F47D20","#3E3E3E","#300961","#023452","#99BAC7"))
...
pubmed_keyword_frequency <- ggplot()+
    ...
    scale_fill_manual(values=categories_colors$color)+
...
#    mutate(keyword=fct_reorder(keyword,category, .desc=TRUE)) %>% 
    mutate(count_bin=cut(counts, breaks=c(0,200, 500, 1000, 5000, 10000, max(counts,na.rm=T)),
                         labels=c("1-200","200-500", "500-1000", "1000-5000","5000-10000","10000<"))) %>%
    ungroup()
...
pubmed_keyword_per_year_heatmap <- ggplot()+
    ...
    scale_y_discrete(expand=c(0,0))+
    scale_fill_manual(values=c("#f1dfd1","#e4bfa3","#d7a076","#d0905f","#ca8048","#bd611b"))+
    ylab("")+
    xlab("")+
    coord_equal()+
    theme_bw()+
...
keywords_heatmap_long$count_bin <- cut(keywords_heatmap_long$count, 
                                       breaks=c(0,5,50, 100, 500, 1000, 2000,max(keywords_heatmap_long$count)),
                                       labels=c("1-5","5-50", "50-100", "100-500","500-1000","1000-2000","2000<"))
...
p <- ggraph(coword_graph_tidy,layout = 'stress') +
        ...
        scale_color_manual(values=categories_colors$color)+
